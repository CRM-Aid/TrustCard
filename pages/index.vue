  ...
  >
    ...
    <transition name="drop">
      <div
        ...
        class="fixed bottom-0 right-2 w-full z-30 justify-end py-3 items-end flex md:hidden"
      >
        <button
          class="px-4 py-2 font-extrabold rounded-full focus:outline-none select-none"
          :class="showPreview ? 'bg-gray-700' : 'bg-emerald-600'"
          @click="!opening && togglePreview()"
        >
          <span v-if="showPreview">Close Preview</span>
          <svg
            v-else
    ...
    <div class="px-4">
      <div class="flex items-start justify-between pt-8">
        <NuxtLink to="/">
          <img
            ...
            class="logo w-16"
            alt="TrustCard- An Open-Source Digital Business Card Generator"
          />
        </NuxtLink>
        <a
          class="font-extrabold tracking-wide leading-none shrink-0 p-5 border-2 text-white border-gray-700 rounded hover:bg-gray-700 transition-colors duration-200"
          href="https://app.crmaid.ai"
          target="_blank"
          rel="noreferrer"
          >Login</a
        >
      ...
      <p class="mt-8 text-lg md:text-xl w-full md:w-3/4 text-gray-200">
        <b> TrustCard </b> helps you create beautiful, responsive
        HTML&#8209;based digital business cards that can be hosted on your
        website.
      </p>
      ...
      <div class="mt-4 flex flex-wrap items-center">
        ...
        <a
          ...
          >View demo</a
        >
        <!-- TODO: Add this in future update with some fixes -->
        <!-- <div class="relative inline-block text-left">
          <button
            @click="toggleDropdown"
            class="flex items-center justify-between p-4 mt-2 text-white bg-gray-700 rounded-lg shadow-lg hover:bg-gray-600 focus:bg-gray-600 transition-all duration-300 ease-in-out"
          >
          ...
          <div
            v-if="isDropdownOpen"
            class="absolute mt-2 p-4 bg-white border border-gray-300 rounded-lg shadow-xl flex"
          >
            <div class="border-gray-200 p-2">
              <h3 class="text-gray-600 font-semibold text-sm mb-2">Themes</h3>
              <nuxt-link
                to="/demo/theme1"
                target="_blank"
      ...
      <div class="px-4 mt-32">
        ...
        <div id="step-4" class="mt-16">
          ...
          >
            <input
              ...
              placeholder="Search an action"
              class="px-4 mb-6 w-full h-12 bg-black placeholder-gray-600 rounded border border-transparent transition-colors duration-200 focus:outline-none focus:border-gray-600 hover:border-gray-600"
              @keydown.esc="clearFilterActions"
              @keypress.enter="
        ...
        <div id="step-5" class="mt-16">
          ...
          <div class="stepC">
            ...
            <p class="mt-6 border p-4 rounded border-gray-700 text-gray-400">
              Note: When you embed a YouTube iframe and download the ZIP file,
              opening index.html locally may show a "Video Unavailable" error.
              This happens because YouTube restricts iframes on file:// URLs.
              However, once hosted, the iframe will work perfectly.
            </p>
          </div>
        </div>
        <div id="step-6" class="mt-16">
          ...
          <div class="stepC mt-6">
            <div class="flex items-center">
              <div
                class="relative group inline-block w-24 h-12 mr-3 align-middle select-none transition duration-200 ease-in bg-gray-700 rounded-full hover:bg-gray-600 focus:bg-gray-600 cursor-pointer focus:outline-none"
                :class="{
              ...
              >
                <transition name="slide">
                  <input
                    ...
                    class="toggle-switch absolute block w-10 h-10 m-1 rounded-full border-4 border-transparent appearance-none cursor-pointer transition-colors duration-200 focus:outline-none bg-white"
                    tabindex="-1"
                  />
                </transition>
              </div>
        ...
        <div id="step-7" class="mt-16">
          ...
          <div class="stepC mt-3 flex flex-wrap">
            <button
              @click="changeTheme(1)"
              class="w-16 h-12 px-2 rounded mt-3 mr-3 font-extrabold focus:outline-none transition-colors duration-200"
              :class="
                theme == 1
                  ? 'bg-emerald-600'
            ...
            <button
              @click="changeTheme(2)"
              class="px-2 rounded mt-3 mr-3 font-extrabold focus:outline-none transition-colors duration-200"
              :class="
                theme == 2
                  ? 'bg-emerald-600'
                  : 'bg-gray-700 hover:bg-gray-600 focus:bg-gray-600'
    ...
    downloadPackage() {
      if (this.downloadChecked) {
        ...
        setTimeout(async () => {
          ...
          // Create GHL optimized version (single file)
          let ghlEl = new DOMParser().parseFromString(
            this.$refs.html.$refs.html.outerHTML,
            'text/html'
          )

          // Add styles directly to GHL version
          let styleTag = document.createElement('style')
          styleTag.textContent = `
        .bgCover {
          background-color: ${this.colors.logoBg.color} !important;
        }
      `
          ghlEl.querySelector('head').appendChild(styleTag)

          // Add Theme1 styles
          let themeStyleTag = document.createElement('style')
          themeStyleTag.textContent = theme // Assuming Theme1 is already defined in your script
          ghlEl.querySelector('head').appendChild(themeStyleTag)

          // Add QR code script directly to GHL version
          let qrScriptTag = document.createElement('script')
          qrScriptTag.textContent = QRCode
          ghlEl.querySelector('body').appendChild(qrScriptTag)

          // Convert and embed images for GHL version
          for (const key in this.images) {
            if (this.images[key].url) {
              const base64Data = await fileToBase64(this.images[key].resized)
              // Update all matching image sources in the GHL HTML
              ghlEl.querySelectorAll(`img[src*="${key}"]`).forEach((img) => {
                img.src = base64Data
              })
            }
          }

          // Handle featured content for GHL version
            ...
            for (const item of this.featured) {
              for (const content of item.content) {
                if (content.contentType == 'media') {
                  const base64Data = await fileToBase64(content.file)
                  // Update media sources
                  ghlEl
                    .querySelectorAll(
                      `[src*="${this.getTitle(content.title)}"]`
                    )
                    .forEach((el) => {
                      el.src = base64Data
                    })
                  ...
                  if (
                    content.type.match(/music|document/gi) &&
                    !content.info &&
                    content.cover
                  ) {
                    const coverBase64 = await fileToBase64(content.cover)
                    // Update cover image sources
                    ghlEl
                      .querySelectorAll(
                        `[src*="${this.getTitle(content.title)}"]`
                      )
                  ...
                  const base64Data = await fileToBase64(content.image.file)
                  // Update product image sources
                  ghlEl
                    .querySelectorAll(
                      `[src*="${this.getTitle(content.image.title)}"]`
                    )
                    .forEach((el) => {
                      el.src = base64Data
                    })
          ...
          // Handle original version setup (unchanged)
          ...
          // Add modal and media handler scripts to both versions
          ...
          ghlEl.querySelector('body').appendChild(modals.cloneNode(true))
          ...
          if (this.featured.length) {
            ...
            ghlEl
              .querySelector('body')
              .appendChild(mediaHandler.cloneNode(true))
          }

          // Handle tracking scripts for both versions
          let tracker = this.getTrackingCode()
          if (tracker) {
            while (tracker.firstChild) {
              el.head.appendChild(tracker.firstChild.cloneNode(true))
              ghlEl.head.appendChild(tracker.firstChild.cloneNode(true))
            }
          }
          ...
          // Create a hidden div to store vCard data in the GHL HTML
          const vCardDataDiv = document.createElement('div')
          vCardDataDiv.style.display = 'none'
          vCardDataDiv.id = 'vCardData'
          vCardDataDiv.textContent = vCardData
          ghlEl.querySelector('body').appendChild(vCardDataDiv)

          // Find and modify the vCard download link/button in GHL version
          const vCardElements = ghlEl.querySelectorAll('[href$=".vcf"]')
          vCardElements.forEach((element) => {
            element.removeAttribute('href')
            element.setAttribute('href', '#')
            element.setAttribute('onclick', 'return downloadVcard(event)')
          })

          // Add vCard download functionality directly in GHL HTML
          let vCardScript = document.createElement('script')
          vCardScript.textContent = `
        function downloadVcard(event) {
          event.preventDefault();
          const vCardData = document.getElementById('vCardData').textContent;
          const blob = new Blob([vCardData], { type: 'text/vcard;charset=utf-8' });
          const link = document.createElement('a');
          link.href = URL.createObjectURL(blob);
          link.download = "${this.username}.vcf";
      ...
      `
          ghlEl.querySelector('body').appendChild(vCardScript)
          ...
          let ghlHtml = new Blob(
            [`<!DOCTYPE html>${ghlEl.documentElement.outerHTML}`],
            {
              type: 'text/html',
            }
          )
          let css = new Blob([theme], { type: 'text/css' })
          let vCard = new Blob([vCardData], { type: 'text/plain' })
          let qrScript = new Blob([QRCode], { type: 'application/javascript' })
          let guide = new Blob(
            [
              '<html><head><meta http-equiv="refresh" content="0; url=https://trustcard.crmaid.ai/hosting-guide" /></head></html>',
            ],
            {
              type: 'text/html',
            }
          ...
          let zip = new JSZip()
          // Original version with separate files
          ...
          // Add original version featured content
          if (hasFeaturedContent) {
          ...
          // GHL version (single file with embedded assets)
          zip.folder(username).folder('GHL(Bundle)').file('index.html', ghlHtml)
          ...
          zip
            ...
            .then(function (zip) {
              saveAs(zip, `${username}'s TrustCard.zip`)
            })
